...
def find_customer(name_or_id: str) -> dict | None:
    ...
    try:
        ...
        docs    = results.get("documents", [[]])[0]
        # Look for a chunk containing the name/ID
        needle  = name_or_id.lower()
        for doc in docs:
            if needle in doc.lower() or any(w in doc.lower() for w in needle.split()):
                return {"record": doc}
    except Exception as e:
            ...
            if "bytes" in msg and msg["bytes"]:
                ...
                async with processing:
                    ...
                    if stage == "ask_name":
                        ...
                        if customer:
                            session["customer_name"] = transcript
                            session["customer_data"] = customer
                            session["stage"]         = "ask_pin"
                            # Extract first name for friendly address
                            first = transcript.strip().split()[0].title()
