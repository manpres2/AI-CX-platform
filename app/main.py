...
def synthesize(text: str, voice: str | None = None, speed: float = 1.0) -> bytes:
    ...
    return _shared_tts.synthesize("kokoro", text, voice=voice,
                                  lang_code="a", repo_id=KOKORO_REPO_ID, speed=speed)
...
async def voice_ws(ws: WebSocket):
    ...
    # Streaming utterance state. Binary frames are accumulated while VAD is open;
    # partial snapshots may prepare a turn, but only audio_end may release a reply.
    ...
    async def stream_tts(text: str, label: str) -> float:
        ...
        for index, chunk in enumerate(chunks):
            ...
            log.info("STEP 6 ▶ TTS chunk %d/%d ready (%.2fs): %.1f KB (%.1fs audio)",
    ...
    async def say(text: str, msg_type: str = "reply", _nudge: bool = False) -> float:
        ...
        try:
            ...
            if interrupted.is_set():
                log.info("STEP 6 ▶ TTS interrupted after %.2fs — remaining chunks skipped", time.time() - ts)
                await ws.send_json({"type": "barge_in_ack"})
            else:
                log.info("STEP 6 ▶ All TTS chunks sent in %.2fs (%.1fs audio)",
